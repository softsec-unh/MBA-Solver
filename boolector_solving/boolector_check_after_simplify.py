...
def boolector_check_time(sourcefilename, timeout, bv=64):
    ...
    Raises:
        None.
    """
    desfilename = "{source}.boolector.verify.{bv}bit.after.simplify.txt".format(source=sourcefilename, bv=bv)
    fwrite = open(desfilename, "w")
    comment = "#complex,groundtruth,simcomplex,simground,boolectorres,verificationtime\n"
    ...
    with open(sourcefilename, "r") as data:
        ...
        for line in data:
            ...
            if "#" not in line:
                ...
                groundSimStr = expreStrList[3]

                start = time.time()
                #res = verify_mba_unsat(sourceSimStr, groundSimStr, bv)
                res = Value('i', 0)
                p = Process(target=verify_mba_unsat, args=(sourceSimStr, groundSimStr, bv, res,))
                p.start()
                ...
                # print linenum, sourceExpreStr, groundExpreStr, sourceSimStr, groundSimStr, result[res.value], elapsed

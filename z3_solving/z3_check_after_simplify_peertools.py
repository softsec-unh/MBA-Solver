...
def z3_check_time(sourcefilename, timeout, bv=64):
    ...
    print("#complex,groundtruth,simcomplex,z3res,verificationtime", file=fwrite)
    ...
    with open(sourcefilename, "r") as data:
        ...
        for line in data:
            ...
            if "#" not in line: 
                ...
                p = Process(target=verify_mba_unsat, args=(sourceSimStr, groundtruthStr, bv, res,))
                p.start()
                ...
                elapsed = end - start
                print(sourceExpreStr, groundtruthStr, sourceSimStr, result[res.value], elapsed, sep=",", file=fwrite, flush=True)

                # print(linenum, sourceExpreStr, groundtruthStr, sourceSimStr, result[res.value], elapsed)

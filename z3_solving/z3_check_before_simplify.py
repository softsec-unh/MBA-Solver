...
def z3_check_time(sourcefilename, timeout, bv=64):
    ...
    """
    desfilename = "{source}.z3.verify.{bv}bit.before.simplify.txt".format(source=sourcefilename, bv=bv)
    fwrite = open(desfilename, "w")
    print("#complex,groundtruth,z3res,verificationtime", file=fwrite)
    ...
    with open(sourcefilename, "r") as data:
        ...
        for line in data:
            ...
            if "#" not in line:
                ...
                # res = verify_mba_unsat(sourceExpreStr, groundtruthStr, bv)
                p = Process(target=verify_mba_unsat, args=(sourceExpreStr, groundtruthStr, bv, res,))
                p.start()
                ...
                elapsed = end - start
                print(sourceExpreStr, groundtruthStr, result[res.value], elapsed, sep=",", file=fwrite, flush=True)

                # print(linenum, sourceExpreStr, groundtruthStr, result[res.value], elapsed)
